...
const Magnifier = (props) => {
  ...
  const makeDraggable = () => {
    ...
    const currentWindow = dragHandler && dragHandler.ownerDocument.defaultView || window;
  ...
  useEffect(() => {
    const currentWindow = magnifierRef.current && magnifierRef.current.ownerDocument ? magnifierRef.current.ownerDocument.defaultView : window;
    if(active) {
      prepareContent();
